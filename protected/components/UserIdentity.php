		...
		$user = User::model()->findByAttributes(array(
         ...
         if ($user === null) {
             ...
             $password = ($this->autoLogin == false)
                 ? md5($this->username . $this->password . $user->registered_date)
                 : $this->password;
			//var_dump($user);die();
             if ($user->password !== md5($this->password)) {
                 ...
                 $this->_id = $user->id;
                 if ($user->lastvisited_date === NULL) {
                     $lastLogin = new CDbExpression('NOW()');
                 } else {
                     $lastLogin = $user->lastvisited_date;
                 }
                 ...
                 $this->setState('lastvisited_date', $lastLogin);
                 $this->errorCode = self::ERROR_NONE;
             }
         }
         return !$this->errorCode;
